...
# ╔═╡ 0dd83a24-011d-11ec-3b44-79b44660556d
begin
	using Symbolics
	using Plots
end

# ╔═╡ 118474e8-9039-404c-acf5-764a24c062a5
@variables γ r₁

# ╔═╡ 48dd580b-1cc5-44e1-9c78-df657747504c
r(s) = s == 1 ? r₁ : 0

# ╔═╡ 8f1b0a3a-7ff5-4480-8d01-39c33e7f665f
# γ(s) = γ₁

# ╔═╡ 71d977f9-252f-4706-97f1-4fc0b4d110b8
@register r(s)

# ╔═╡ 3f523a07-ce5d-48a8-a3ae-6395b3165d25
v_5_0(s) = (sum(γ^(j) * r(mod1(s + j + 1, 5)) for j in 0:4))/(1-γ^5)

# ╔═╡ 23bc0d2b-cbe5-49b4-af18-7233b830498b
function v(n, i, s) 
	if i == 0
		(sum(γ^(j) * r(mod1(s + j + 1, 5)) for j in 0:4)) * 1/(1-γ^n)
	else
		expand((sum(γ^j * v(n, i-1, mod1(s+j+1, n)) for j in 0:(n-1))) * 1/(1-γ^n))
	end
end

# ╔═╡ 8133dddb-eb66-4d35-a96d-32f679772a69
function v_dynamic(n, I)
	dyn = Matrix{Num}(undef, I, n)
	for i in 1:I
		if i == 1
			for s in 1:n
				dyn[i, s] = expand(sum(γ^(j) * r(mod1(s + j + 1, n)) for j in 0:n-1))
			end
		else
			for s in 1:n
				dyn[i, s] = expand(sum(expand(γ^(j) * dyn[i-1, mod1(s + j + 1, n)]) for j in 0:n-1))
			end
		end
	end
	dyn
...
# ╔═╡ e9eeaee2-fdd4-498e-866b-07038ae729fb
let
	vs = v_dynamic(10, 10)
	substitute.(vs, (Dict(r₁ => 1, γ=>0.9), )) .* 0.1.^(0:9)
end

# ╔═╡ c4becd6b-8606-4e2f-8677-75fc02f38d9c
let	
	vs = v_dynamic(5, 10)
	substitute.(vs, (Dict(r₁ => 1, γ=>1), ))
end

# ╔═╡ c2da6690-a87d-48ae-8de1-617d35fd2464
vs = v_dynamic(5, 10)
...
git-tree-sha1 = "9d6e12adb9d8e059fd915ebcda2a5a84a440382d"
