...
# ╔═╡ dc1f18f5-35af-4e2f-bc09-4ae19d7ab55b
using Plots

# ╔═╡ 50f982f0-e874-11ed-3731-9ba7f15bd6ad
...
# ╔═╡ 17550fe7-21d2-495f-9c05-182c29b6a363
import MinimalRLCore, RecipesBase, Colors, ProgressLogging # CompGVFs.jl dependencies

# ╔═╡ b62030d0-6f41-4b87-8eee-6e46d0acf95d
import PlutoUI

# ╔═╡ 06f2831d-11c9-4b30-b064-a8714dda2588
import MarkdownLiteral: MarkdownLiteral, @mdx

# ╔═╡ 8cd5ed4a-8790-416b-97e8-fb1056cd85f6
import Random

# ╔═╡ b90ac18b-45b1-40ce-8690-ea018c90fa2a
PlutoUI.TableOfContents()

# ╔═╡ 9058fa47-10eb-424d-b751-60ddec32d0dd
CompGVFs = ingredients("../src/CompGVFs.jl").CompGVFs.CompGVFs

# ╔═╡ c6fcd0c2-23d3-49d4-8c34-c7a757b780f9
function heatmap_valuefunction(env, p::AbstractVector{<:AbstractFloat}; pkwargs...)
	width, height = size(env)
	heatmap(reshape(p, width, height)[:, end:-1:1]'; pkwargs...)
end

# ╔═╡ 1ae7f971-c24a-40c6-b9df-81d9f1b436df
function heatmap_policy(env, p::AbstractVector{<:AbstractVector})
	plt_up = heatmap_valuefunction(env, getindex.(p, 1))
	title!(plt_up, "Up")
	plt_right = heatmap_valuefunction(env, getindex.(p, 2))
	title!(plt_right, "Right")
	plt_down = heatmap_valuefunction(env, getindex.(p, 3))
	title!(plt_down, "Down")
	plt_left = heatmap_valuefunction(env, getindex.(p, 4))
	title!(plt_left, "Left")
...
# ╔═╡ c3b37766-5959-420d-a02d-2bdac499f54f
let
	env = CompGVFs.NoWallsGridWorld(30, 30)
	env.state = [15, 15]
	MinimalRLCore.get_state(env)
end

# ╔═╡ 575fbf3d-5232-4444-8a28-38f2c41acd60
b_pred, bdemon = let
	env = CompGVFs.NoWallsGridWorld(30, 30)
	env_size = size(env)
	env_feat_size = env_size[1] * env_size[2] 
	bdemon = CompGVFs.BDemon(
		CompGVFs.FeatureCumulant(435), 
		CompGVFs.ϵGreedy(0.1),
		CompGVFs.TerminatingDiscount(0.9, 435))
	learner = CompGVFs.QLinearLearner(
		env_feat_size, 4, bdemon, CompGVFs.Qλ(0.01, 0.9))
	CompGVFs.behavior_offpolicy!(env, learner, 2_000_000), learner
end

# ╔═╡ 9e0c4900-4314-4e4a-b30e-7bcd6b368fb3
heatmap_policy(CompGVFs.NoWallsGridWorld(30, 30), b_pred)

# ╔═╡ 322b709e-5ca6-4c7d-a27f-949e1500fb39
fr_bd_hrd, fr_bd_p = let
	num_steps = 2_000_000
	lu = CompGVFs.TDλ(0.005, 0.9)
	γ = 0.9
	env = CompGVFs.NoWallsGridWorld(30, 30)
	env_size = size(env)
	env_feat_size = env_size[1] * env_size[2]
	horde = [
		[CompGVFs.GVF(
		 	CompGVFs.FeatureCumulant(435),
			bdemon,
			CompGVFs.TerminatingDiscount(γ, 435))
		];
		[CompGVFs.GVF( 
	...
	horde, p = CompGVFs.gridworld_experiment!(
		env,
		learner,
		num_steps)
	horde, p
...
# ╔═╡ bec482bf-25bb-4353-a0c1-ab77deacc58b
@mdx """
...
# ╔═╡ 0403996a-993c-47dd-b6f5-487b910c9b97
_ncp_horde_plts = [heatmap_valuefunction(CompGVFs.NoWallsGridWorld(30, 30), getindex.(fr_bd_p, i), title="GVF $(i)") for i in 1:12];

# ╔═╡ e74589b8-58d5-42eb-a1cd-158d25a39f26
_ncp_horde_plts[_ncp_horde_idx]

# ╔═╡ 64f260a6-d41d-436d-a20f-46b18ddb5976
